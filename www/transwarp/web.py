...
"""
import threading,re,datetime,types,os,re,cgi,sys,time,functools,mimetypes,logging,traceback,urllib
from db import Dict
import utils
...
class Response(object):
    ...
    def header(self,name):
        #获取Response Header里单个Header的值，非大小写敏感
        key = name.upper()
        if key not in _RESPONSE_HEADER_DICT:
            key = name
...
def _build_pattern_fn(pattern):
    """
    传入需要匹配的字符串： URL
    返回一个函数，该函数接收一个字符串参数，检测该字符串是否符合pattern
    """
